...
const update = async (userModel, req, res) => {
  ...
  if (body.section == "1") {
    ...
    let { name, headLine, dob, country, state, city } = value;
    ...
    if (!body.describesAnimalType) {
      return sendResponse(
        ...
        false,
        null,
        "describesAnimalType is required"
      );
    }
    updates = {
    ...
    const preferredUnits =
      body.preferredUnits || currentUser?.preferredUnits || "metric";

    updates = {
      weight: body.weight
        ? normalizeWeight(body.weight, preferredUnits)
        : currentUser?.weight,
      height: body.height
        ? normalizeHeight(body.height, preferredUnits)
        : currentUser?.height,
